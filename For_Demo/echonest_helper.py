...
def getSongs(style,mood,results=15,max_tempo=200.0,min_tempo=80.0,max_danceability=0.8,min_danceability=0.5):
    ...
    print start_year
    songs = song.search(style=style, mood=mood, results=results,min_tempo=min_tempo,max_tempo=max_tempo,min_danceability=min_danceability,max_danceability=max_danceability,buckets=["id:7digital-US", "tracks", "audio_summary"],limit='true',max_duration=240, sort='artist_familiarity-asc', artist_start_year_before=start_year)
...
def getPlaylist(style,mood,likes, previous,results=15,max_tempo=200.0,min_tempo=80.0,max_d=0.8,min_d=0.5):
    ...
    if(len(previous)>0):
        if (likes<-3.3):
            p = playlist.Playlist(type='song-radio', limit='true',variety=0.8, style=style, mood=mood, buckets=["id:7digital-US", "tracks", "audio_summary"], max_danceability=max_d,min_danceability=min_d,max_tempo=max_tempo,min_tempo=min_tempo)
        elif (likes>3.3):
            p = playlist.Playlist(type='song-radio', limit='true',variety=0.8, style=style, mood=mood, buckets=["id:7digital-US", "tracks", "audio_summary"], max_danceability=max_d,min_danceability=min_d,max_tempo=max_tempo,min_tempo=min_tempo)
        else:
            p = playlist.Playlist(type='song-radio', limit='true',variety=0.8, style=style, mood=mood, buckets=["id:7digital-US", "tracks", "audio_summary"], max_danceability=max_d,min_danceability=min_d,max_tempo=max_tempo,min_tempo=min_tempo)
    ...
    songs = []
    songs.append(p.get_current_song())
    songs.append(p.get_next_song())
    return songs
